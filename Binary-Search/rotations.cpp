...
int max_pivotIndex(vector<int> arr, int n) {
    if(arr[0] < arr[n-1] || n == 1) return (n - 1);
    int s = 0, e = n-1;
    int mid = (s + e) / 2;
    int ans = -1;
    if(arr[0] == arr[mid] && arr[mid] == arr[e]) return (n - 1);
    else if(mid && arr[mid] < arr[mid - 1]) return (mid - 1);
    int value = INT_MIN;

    while(s <= e) {
        if((arr[mid] >= arr[mid + 1] && arr[mid] >= arr[mid - 1] && arr[mid] >= value) || (mid == 0 && arr[mid] >= arr[mid + 1] && arr[mid] >= value)) {
            ans = mid;
            value = arr[ans];
            s = mid + 1;
        } else if(arr[n - 1] >= arr[mid]) e = mid - 1;
        else s = mid + 1;
